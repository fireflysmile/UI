...
@Component({
  selector: 'app-assigned-task-table',
  templateUrl: './assigned-task-table.component.html',
  styleUrls: ['./assigned-task-table.component.scss']
})
export class AssignedTaskTableComponent extends TablePageBaseComponent<TaskItem> implements OnInit {
  // previous start date
  ...
  private _getTasks(start?: Date, end?: Date): void {
    ...
    const sub = this.taskService
      ...
      .subscribe({
        next: res => {
          this._originalData = res.filter(task => task.subStatus !== 'Final Approval');
