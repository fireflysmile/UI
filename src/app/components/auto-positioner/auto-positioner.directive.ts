...
export type TsAutoPositionerVerticalPriority = 'top' | 'bottom' | 'auto';
export type TsAutoPositionerHorizontalPriority = 'left' | 'right' | 'auto';
...
export class AutoPositionerDirective implements OnInit, AfterViewInit, OnDestroy {
  ...
  @Input() set width(width: string | number) {
    this._width = toFloat(width);
  }

  // set max width
  @Input() set maxWidth(width: string | number) {
    this._maxWidth = toFloat(width);
  }
  ...
  @Input() verticalPriority: TsAutoPositionerVerticalPriority = 'auto';
  // horizontal position priority
  // if both left/right positions are available/unavailable,
  // the component should use priority position
  @Input() horizontalPriority: TsAutoPositionerHorizontalPriority = 'auto';
  ...
  private _calculateAvailablePosition(hideBeforeCalculate = false): void {
    // elements
    const hostRect = this.host.getBoundingClientRect() as DOMRect;
    const containerRect = this.container.getBoundingClientRect() as DOMRect;
    const scrollContainerRect = this.scrollContainer.getBoundingClientRect() as DOMRect;
    const hostSize = {
    ...
    if (horizontal === 'left') {
      ...
      this.host.setAttribute('autoPositionedHorizontal', 'left');
    } else {
      x = containerRect.left + this.horizontalGap;
      this.host.setAttribute('autoPositionedHorizontal', 'right');
    }

    // calculate y axis value
    if (vertical === 'top') {
      y = containerRect.top - hostSize.height - this.verticalGap;
      this.host.setAttribute('autoPositionedVertical', 'top');
    } else {
      y = containerRect.top + containerRect.height + this.verticalGap;
      this.host.setAttribute('autoPositionedVertical', 'bottom');
    }
  ...
  ): TsAutoPositionerHorizontalPriority {
  ...
  private _getVerticalPosition(
    ...
    containerRect: DOMRect,
    scrollContainerRect: DOMRect,
  ): TsAutoPositionerVerticalPriority {
